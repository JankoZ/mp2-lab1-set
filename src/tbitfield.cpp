...
int TBitField::operator==(const TBitField &bf) const // сравнение
{
    ...
    else for (int i = 0; i < MemLen; i++) if (pMem[i] != bf.pMem[i]) return 0;
...
int TBitField::operator!=(const TBitField &bf) const // сравнение
{
    ...
    else for (int i = 0; i < MemLen; i++) if (pMem[i] != bf.pMem[i]) return 1;
...
{
    int temp;
    if (BitLen >= bf.BitLen) temp = BitLen;
    else temp = bf.BitLen;

    TBitField tempField(temp);
    for (int i = 0; i < tempField.MemLen; i++) tempField.pMem[i] = bf.pMem[i] | pMem[i];
